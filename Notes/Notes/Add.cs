...
namespace MillionDollarNoteApp
{
    ...
    {
        private SQLiteWrapper api = new SQLiteWrapper("notes_database");
        ...
        public Add(string note_id = null)
        {
            ...
            if (note_id != null)
            {
                ...
                Dictionary<string, string> data = api.singleRecord("notes", cols, "id", note_id);
        ...
        private void add()
        {
            ...
            if (! api.insert("notes", col_tmpl, vals))
            {
        ...
        private void update()
        {
            if ( api.update("notes", "title", "\"" + title + "\"", "id", id) )
            {
                if ( api.update("notes", "body", "\"" + body + "\"", "id", id) )
                {
                    MessageBox.Show("Changes saved!");
                }
                else
